...
| External exchange classifier | Fine-tuned Claude Haiku-family exchange classifier. | CC++ Sections 3, 4, 5.3, 6. | Gemma2 YES/NO exchange classifier, preferably LoRA fine-tuned. | Local substitute. | substitute | Classifier quality and calibration are not comparable to Claude. | Prompt/logit scoring is allowed only as a temporary lower-faithfulness baseline. |
