...
def main() -> None:
    ...
    if args.adversarial_dataset is not None:
        ...
        adv_texts, y_adv = examples_to_text_and_labels(
            adversarial,
            label_key=args.label_key,
        )
    ...
    print(
        f"Val  ROC-AUC: {artifacts.val_result.roc_auc:.4f}  "
        f"TPR@1%FPR: {artifacts.val_result.tpr_at_threshold:.4f}"
    )
    print(
        f"Test ROC-AUC: {artifacts.test_result.roc_auc:.4f}  "
        f"TPR@1%FPR: {artifacts.test_result.tpr_at_threshold:.4f}"
    )
    if adversarial_row is not None:
        print(
            ...
            "  Positives: "
            f"{adversarial_row['positive_predictions']}/"
            f"{adversarial_row['n_examples']}"
        )
