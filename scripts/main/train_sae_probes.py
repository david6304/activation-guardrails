"""Train one logistic probe per SAE layer and select the best layer.

Usage:
    python scripts/main/train_sae_probes.py --config configs/main/main.yaml
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train logistic probes on SAE feature activations."
    )
    parser.add_argument("--config", default="configs/main/main.yaml")
    parser.add_argument("--input-dir", default="artifacts/features/main/sae")
    parser.add_argument(
        "--output-dir",
        default="artifacts/models/main/sae_probes",
        help="Directory for fitted SAE probe artifacts.",
    )
    parser.add_argument(
        "--metrics-dir",
...
def main() -> None:
    ...
    train = load_layer_feature_split(
    ...
    if adversarial_labels_path.exists():
        adversarial = load_layer_feature_split(
            input_dir=args.input_dir,
            split="adversarial",
            layers=layers,
            feature_name="sae_features",
    ...
    for layer in layers:
        ...
        save_artifact(result.probe, output_dir / f"layer_{layer}_sae_probe")
        ...
        print(
            f"SAE layer {layer:2d}: val ROC-AUC={result.val_result.roc_auc:.4f}"
