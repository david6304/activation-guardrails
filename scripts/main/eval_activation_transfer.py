"""Evaluate a frozen dense activation probe on cached transfer activations."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate a frozen dense activation probe on transfer activations."
    )
    parser.add_argument(
        "--input-dir",
        required=True,
        help="Directory with cached transfer activations.",
    )
    parser.add_argument(
        "--split",
        default="test",
        help="Split name to evaluate from the activation cache.",
    )
    parser.add_argument(
        "--probe-dir",
        default="artifacts/models/refusal/activation_probes",
        help="Directory with fitted plain-text dense probes.",
    )
    parser.add_argument(
        "--plain-metrics",
        default="results/refusal/metrics/probe_metrics.json",
        help="Plain-text probe metrics JSON used to recover best layer and threshold.",
    )
    parser.add_argument(
        "--output",
        required=True,
...
def main() -> None:
    ...
    dataset = load_activation_split(
        input_dir=args.input_dir,
        split=args.split,
        layers=[best_layer],
    )
    probe = load_artifact(Path(args.probe_dir) / f"layer_{best_layer}_probe.joblib")
