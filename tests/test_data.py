...
def test_build_wildjailbreak_refusal_dataset_preserves_harmfulness_source_labels(
    tmp_path,
):
    path = tmp_path / "wildjailbreak.jsonl"
    _write_wildjailbreak_jsonl(path, _wj_records())
    ...
    assert {example.source for example in dataset} == {"wildjailbreak"}
    ...
    assert split_counts == {"train": 6, "val": 3, "test": 3}
    assert split_label_counts[("train", 1)] == split_label_counts[("train", 0)]
    assert abs(split_label_counts[("val", 1)] - split_label_counts[("val", 0)]) <= 1
    assert abs(split_label_counts[("test", 1)] - split_label_counts[("test", 0)]) <= 1


def test_build_wildjailbreak_refusal_adversarial_set_uses_harmful_source_label(
    tmp_path,
):
    path = tmp_path / "wildjailbreak.jsonl"
    _write_wildjailbreak_jsonl(path, _wj_records())

    test_set = build_wildjailbreak_refusal_adversarial_set(
        wildjailbreak_path=path,
        n_adversarial=4,
    )

    assert len(test_set) == 4
    assert all(example.split == "test" for example in test_set)
    assert all(example.label == 1 for example in test_set)
    assert all(example.source_label == "harmful" for example in test_set)
...
def _write_alpaca_json(path, instructions):
    ...
    records = [
        {"instruction": inst, "input": "", "output": "..."} for inst in instructions
    ]
...
def test_make_fixed_splits_assigns_correct_counts(tmp_path):
    ...
    dataset = make_fixed_splits(
        examples, train_size=120, val_size=40, test_size=880, seed=42
    )
...
def test_build_mvp_dataset_and_write_round_trip(tmp_path):
    ...
    write_jsonl(
        ...
        + [
            {"id": 100 + i, "prompt": f"unsafe {i}", "label": "unsafe"}
            for i in range(4)
        ],
    )
